  ...
  return (
    <header className="sticky top-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm shadow-md z-50 transition-colors duration-300">
      <div className="container mx-auto flex justify-between items-center h-20 px-4">
        <a href="#home" className="flex-shrink-0" onClick={() => setIsMenuOpen(false)}>
          <img 
            src={logo} 
        ...
        <nav className="hidden lg:flex items-center space-x-2">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="px-5 py-3 font-medium text-primary-blue dark:text-gray-200 text-sm uppercase tracking-wide rounded-lg transition-all duration-300 hover:bg-accent-red hover:text-white dark:hover:text-white hover:scale-110 hover:shadow-lg"
            >
              {item.label}
            </a>
          ))}
      ...
      <div className={`lg:hidden bg-white dark:bg-gray-900 transition-all duration-300 ease-in-out overflow-hidden ${isMenuOpen ? 'max-h-96' : 'max-h-0'}`}>
        <nav className="py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col items-center">
          {navItems.map((item) => (
            <a key={item.href} href={item.href} onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 text-primary-blue dark:text-gray-200 font-medium w-full text-center hover:bg-gray-100 dark:hover:bg-gray-800 rounded-md">
              {item.label}
            </a>
          ))}
